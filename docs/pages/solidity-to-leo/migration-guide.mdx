...
- **Leo 隐私** ： `public` 与 `private` 决定了实际的密码学隐私级别，即数据是存储在链上还是链下的 [records](../../../concepts/fundamentals/records/index.html) 中
...
目标程序和函数名称是 **公开的** 电路输入，避免使用敏感的私有数据来选择动态调用目标。详细信息请参阅 [动态调度指南](../../aleo/language/index.html#dynamic-dispatch) 。
...
> **信息**：CREATE2 是 Solidity 中的一个特殊操作码，用于确定性计算创建合约的地址，请参阅 [Solidity 文档](https://docs.soliditylang.org/en/v0.8.30/control-structures.html#salted-contract-creations-create2) 了解更多信息。
...
- **独立程序** ：Aleo Instructions 可以作为独立程序编写在 `.aleo` 文件中，从而实现对程序执行和电路设计的精细低层级控制。有关 Aleo Instructions 的更多详情，请参阅 [Aleo Instructions 概述](../../aleo/aleo/index.html) 。
